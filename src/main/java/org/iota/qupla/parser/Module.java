...
import org.iota.qupla.context.AnyNullContext;
...
import org.iota.qupla.statement.ImportStmt;
import org.iota.qupla.statement.LutStmt;
import org.iota.qupla.statement.TemplateStmt;
...
public class Module extends BaseExpr
{
  ...
  private static String getPathName(final File file)
  {
    try
    {
      ...
      {
        throw new CodeException(null, "Not in project folder: " + file.getPath());
      }
    ...
    {
      e.printStackTrace();
      throw new CodeException(null, "Cannot getCanonicalPath for: " + file.getPath());
    }
  }
  ...
  public static Module parse(final String name)
  {
    ...
    {
      throw new CodeException(null, "Invalid module name: " + name);
    }
    ...
    {
      throw new CodeException(null, "Import dependency cycle detected");
    }
  ...
  {
    throw new CodeException(null, "clone WTF?");
  }
  ...
  final public ArrayList<? extends BaseExpr> entities(final Class classId)
  {
    ...
    throw new CodeException(null, "entities WTF?");
  }
  ...
  public void eval(final CodeContext context)
  {
    for (final LutStmt lut : luts)
    {
      context.evalLutDefinition(lut);
    }

    for (final FuncStmt func : funcs)
    {
      context.evalFuncSignature(func);
    }

    for (final FuncStmt func : funcs)
    {
      context.evalFuncBody(func);
    }
